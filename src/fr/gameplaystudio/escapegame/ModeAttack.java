...
public class ModeAttack {
	...
	public void reponseModeAttack(){// ici c'est ma méthode pour calculer et envoyer la réponse du mode Attack
		...
		ArrayList<String> chiffreClientAndResult = mc.calculModeAttack();
		logger.debug("Après // reçois la méthode calcul du mode attaque");
		
		// ici j'affiche le résultat.
		System.out.println("Proposition : "+chiffreClientAndResult.get(0)+" -> Reponse : " + chiffreClientAndResult.get(1));
		...
		if(egal.toString().equals(chiffreClientAndResult.get(1))) {// je rajoute le toString pour qu'il transforme le stringbuilder en string
