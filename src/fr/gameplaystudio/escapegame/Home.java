...
public class Home {
	...
	public void home() {
		...
		System.out.println("Pour le mode de jeu Attaque taper 1, pour le mode de jeu Défense taper 2, pour le mode de jeu Duel taper 3");
		...
		while(isNan == false) {// je fais une boucle tant que mon boolean est false
			...
			try {
				...
				while(isNotCorrect == true) {// je fais une boucle tant que mon boolean est true
					...
					if(nb != 1 && nb != 2 && nb != 3) {// si ma saisi est différent de 1 2 et 3 je n'arrête pas la boucle
						System.out.println("Vous n'avez pas tapé 1 2 ou 3.");
						nb = sc.nextInt();
					}
					if(nb == 1 || nb == 2 || nb == 3) {	// si ma sais est égale a 1 2 ou 3 j'arrête ma boucle en déclarant is Not Correct en false
