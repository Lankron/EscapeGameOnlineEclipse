...
public class ModeCalcul {
	...
	public ArrayList<String> calculModeAttack() {
		...
		return chiffreClientAndResult;// je retourne mon tableau d'arraylist
		...
		public ArrayList<String> calculModeDefense() {
			...
			String plus = "+";
				...
				for(int i = 0; i < chiffreOrdinateurModeDefense.size(); i++) {// je fais une boucle par rapport a la taille du tableau du chiffre de l'ordinateur et ça pour éviter de refaire des conditions.
					...
					if(moin.equals(propoClient.get(i))){// ici je compare mes strings plus haut par rapport au string de l'utilisateur.
						...
						if(chiffreOrdinateurModeDefense.get(i) != 0) {
							...
							chiffreOrdinateurModeDefense.set(i, rand.nextInt(tabplus.get(i) - tabmoins.get(i) + 1) + tabmoins.get(i));// ici je fais mon calcul si c'est - le chiffre cherchera entre mon tableau moins et mon tableau plus.
				...
				StringBuilder egal = new StringBuilder();
				for(int i = 0; i< config.chiffreCombi();i++)// je fais une boucle pour éviter de me répéter
				{
				...
				propoClientAndChiffreOrdinateur.add(egal.toString());// j'ajoute dans l'arraylist plusieur égale en fonction de la taille du tableau de mes chiffres pour faire une comparaison dans uen autre classe
				logger.info("fin du calcul du mode défense");
			
			return propoClientAndChiffreOrdinateur;// ici je retourne mon ArrayList
		...
		public String affichelecodeComputerModeAttack(){
			...
			return chiffreOrdinateurEnstring.toString();// je retourne mon résultat en string pour pouvoir l'afficher facilement
